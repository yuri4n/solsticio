    ...
    <div class="card text-left mb-3">
      <div class="card-body">
        ...
        <table class="table table-striped">
          ...
          <tbody>
            <tr v-for="classified in classifieds" :key="classified.id">
              <th scope="row">{{ classified.id }}</th>
              <td>
                <a href="#" class="btn btn-link">{{classified.title}}</a>
              </td>
              <td>
                <a href="#" class="btn btn-link">{{classified.category_id}}</a>
              </td>
              <td>
                <button class="btn btn-success" v-on:click="updateClassified(classified.id)">Aprobar</button>
              </td>
              <td>
                <button
                  class="btn btn-danger"
                  v-on:click.prevent="deleteClassified(classified)"
                >Rechazar</button>
              </td>
            </tr>
          </tbody>
    ...
    <nav aria-label="Page navigation example">
      <ul class="pagination">
        <li v-bind:class="[{disabled: !pagination.prev_page_url}]" class="page-item">
          <a class="page-link" href="#" @click="readClassifieds(pagination.prev_page_url)">Anterior</a>
        </li>
        ...
        <li v-bind:class="[{disabled: !pagination.next_page_url}]" class="page-item">
          <a class="page-link" href="#" @click="readClassifieds(pagination.next_page_url)">Siguiente</a>
        </li>
      </ul>
    </nav>
  </div>
