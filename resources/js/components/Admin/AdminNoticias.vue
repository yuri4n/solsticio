        ...
        <div class="card text-left mb-3">
            <div class="card-body">
                <div class="row justify-content-between mb-4">
                    ...
                    <div class="col-sm-2">
                        <button type="button" class="btn btn-primary" data-toggle="modal" data-target=".bd-example-modal-lg">Crear Noticia</button>
                    </div>    
                </div>
                <table class="table table-striped">
                    ...
                    <tbody>
                        <tr v-for="post in posts" :key="post.id">
                            ...
                            <td>{{ post.title }}</td>
                            <td><button class="btn btn-warning" v-on:click.prevent="editPost(post)">Editar</button></td>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="POST" v-on:submit.prevent="createPost()">
                            <div class="form-group">
                                <label for="title">Título</label>
                                <input type="text" name="" id="title" v-model="newTitle" class="form-control" autocomplete="off" aria-describedby="helpId">
                            </div>
                            <div class="form-group">
                            ...
                            <button type="submit" v-on:click="status='DRAFT'" class="btn btn-secundary btn-md btn-block">Guardar como borrador</button>
                            <button type="submit" v-on:click="status='PUBLISHED'" class="btn btn-primary btn-md btn-block">Crear</button>
                        </form>
                    </div>
                </div>
            </div>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Editar Noticia {{ fillPost.id }}</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form action="POST" v-on:submit.prevent="updatePost(fillPost.id)">
                            <div class="form-group">
                                <label for="title">Título</label>
                                <input type="text" name="" id="title" v-model="fillPost.title" class="form-control" autocomplete="off" aria-describedby="helpId">
                            </div>
                            <div class="form-group">
                                <label for="file">Sube una imagen</label>
                            ...
                            <div class="form-group">
                                <label for="excerpt">Resume</label>
                                <textarea class="form-control" id="excerpt" v-model="fillPost.excerpt" rows="3"></textarea>
                            </div>
                            <div class="form-group">
                                <label for="body">Contenido</label>
                                <vue-editor v-model="fillPost.body" id="body"></vue-editor>
                            </div>
                            <button type="submit" v-on:click="status='PUBLISHED'" class="btn btn-primary btn-md btn-block">Actualizar</button>
                        </form>
                    </div>
                </div>
            </div>
        ...
        deletePost(post) {
            ...
            if(confirmacion) {
                axios.delete(url).then(response => {
                    this.alert('warn',`La noticia ${post.id} ha sido eliminada`);
                    this.readPosts(); 
                });
            }           
        },
        slugify() {
            return this.newTitle.toString().toLowerCase()
